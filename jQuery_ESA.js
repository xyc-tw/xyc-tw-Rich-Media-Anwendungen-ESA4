...
jQuery(document).ready(function ($) {
    ...
    function createNewItem(event) {
        ...
        setCheckboxAttributes(cb, uniqueItemID++);
        styleCheckbox(cb);
        ...
        newItem.find('input[type="text"]').pressEnter(function (event) {
            if (editItem) {               //wird ein Listen-Item editiert?
                ...
                fadeInItem(newItem);
                setMenuFuntions(newItem);
            }
        }); 
    }

    function setMenuFuntions($itm) {
        $itm.find('input[type="text"]').off('enterPress');
        $itm.find('input[type="text"]').pressEnter(function (event) {
            if (editItem) {               //wird ein Listen-Item editiert?
                if (event.target == editItem.find('input[type="text"]')[0]) {   //entspricht das Event Target dem editierten Item?
                    unEditModeItem();                                 //Beenden des Editiermodus
                } 
        ...
        $itm.find('.arrow-btn').on('click', function () {
            onArrowClick($(this).closest('li'));
        });
        /*Remove Function*/
        $itm.find('.remove-btn').on('click', function () {
            removeItem($(this).closest('ul.todo-list > li'));
        });
        /*Edit Function*/
        $itm.find('.edit-btn').on('click', function () {
            setEditModeItem($(this).closest('ul.todo-list > li'));
        });
        /*Duplicate Function*/
        $itm.find('.duplicate-btn').on('click', function () {
            var $newItem = duplicateItem($(this).closest('ul.todo-list > li'));
            /*Item Editier Modus beenden vor Erzeugen*/
            if (editItem) {
                unEditModeItem();
